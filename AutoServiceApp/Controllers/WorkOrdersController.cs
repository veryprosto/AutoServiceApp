...
public class WorkOrdersController : ControllerBase
{
    ...
    public WorkOrdersController(AppDbContext context, UserManager<IdentityUser> userManager)
    {
        _context = context;
    ...
    public async Task<ActionResult<WorkOrder>> Create(WorkOrder order)
    {
        ...
        var employeeExists = await _userManager.FindByIdAsync(order.EmployeeId);

        if (!customerExists || !carExists || employeeExists == null)
        {
    ...
    {
        var topEmployees = _context.WorkOrders
            .Where(o => o.Status == OrderStatus.Completed && o.Employee != null)
            .GroupBy(o => o.Employee!.UserName)
            .Select(g => new
            {
                EmployeeName = g.Key ?? "Удаленный сотрудник",
